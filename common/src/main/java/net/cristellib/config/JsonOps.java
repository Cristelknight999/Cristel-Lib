...
/**
 * Mojangs JsonOps, but with jankson and instanceof
 */
public class JsonOps implements DynamicOps<JsonElement> {
    ...
    public <U> U convertTo(final DynamicOps<U> outOps, final JsonElement input) {
        ...
        final Object oPrimitive = primitive.getValue();

        if(oPrimitive instanceof String){
            return outOps.createString(primitive.asString());
        }
        if(oPrimitive instanceof Boolean){
            return outOps.createBoolean(primitive.asBoolean(false));
        }


        final BigDecimal value = primitive.asBigDecimal(new BigDecimal(primitive.asString()));
        try {
    ...
    public DataResult<Number> getNumberValue(final JsonElement input) {
        if (input instanceof JsonPrimitive primitive) {
            ...
            if (compressed && object instanceof String string) {
                try {
        ...
        if (input instanceof JsonPrimitive && ((JsonPrimitive) input).getValue() instanceof Boolean bool) {
            return DataResult.success(bool ? 1 : 0);
        }
    ...
    public DataResult<String> getStringValue(final JsonElement input) {
        if (input instanceof JsonPrimitive primitive) {
            Object object = primitive.getValue();
            if (object instanceof String || object instanceof Number && compressed) {
    ...
    public DataResult<JsonElement> mergeToList(final JsonElement list, final JsonElement value) {
        ...
        if (list != empty() && list instanceof JsonArray array) {
            result.addAll(array);
        }
        result.add(value);
    ...
    public DataResult<JsonElement> mergeToMap(final JsonElement map, final JsonElement key, final JsonElement value) {
        ...
        if (map != empty() && map instanceof JsonObject object) {
    ...
    public DataResult<MapLike<JsonElement>> getMap(final JsonElement input) {
        ...
        return DataResult.success(new MapLike<>() {
            ...
            public JsonElement get(final JsonElement key) {
                if (key instanceof JsonPrimitive primitive && primitive.getValue() instanceof String s) {
                    final JsonElement element = object.get(s);
    ...
    private static final class ArrayBuilder implements ListBuilder<JsonElement> {
        ...
        public DataResult<JsonElement> build(final JsonElement prefix) {
            final DataResult<JsonElement> result = builder.flatMap(b -> {
                ...
                if (prefix != ops().empty() && prefix instanceof JsonArray array1) {
                    array.addAll(array1);
                }
                array.addAll(b);
